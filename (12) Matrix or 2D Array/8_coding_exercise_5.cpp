...
#include<iostream>
using namespace std;
int farToCelcius(int n){
    int c=((n-32)*5)/9;
    return c;
}
void print(int min, int max, int step){
    for(int i=min;i<=max;i+=step){
        cout<<i<<"\t"<<farToCelcius(i)<<endl;
    }
}
int main(){
    int min, max, step;
    cout<<"Enter the minimum temperature in fahrenheit: "<<endl;
    cin>>min;
    cout<<"Enter the maximum temperature in fahrenheit: "<<endl;
    cin>>max;
